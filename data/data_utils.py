    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            if len(temp[0]) < 1:
                ...
                temp[0] = temp[0].split(';')
            temp[2] = temp[2].split(';')
            temp[3] = tokenizer._tokenize(temp[3])
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            if temp[0] in poolset:
                passage[temp[0]] = tokenizer._tokenize(denoise)
...
def load_query(file, tokenizer):  # query_id	query_content
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            query[temp[0]] = tokenizer._tokenize(temp[1])  # {1_1:[query_tokens],}
    return query
...
if __name__ == '__main__':
    ...
    tokenizer = BlenderbotTokenizer.from_pretrained('/data1/ycx/pretrain-models/blenderbot-400M-distill')
    data_path = './wow/blender_data/'
    episodes, query, passage = load_default('wow/raw/wizard_of_wikipedia.answer',
                                                  'wow/raw/wizard_of_wikipedia.passage',
                                                  'wow/raw/wizard_of_wikipedia.pool',
                                                  'wow/raw/wizard_of_wikipedia.qrel',
                                                  'wow/raw/wizard_of_wikipedia.query',
                                                  tokenizer
                                                                   )

    train_episodes, dev_episodes, test_seen_episodes, test_unseen_episodes = split_data('wow', 'wow/raw/wizard_of_wikipedia.split', episodes)


    torch.save(query, data_path + 'query_wow.pkl')
    torch.save(passage, data_path + 'passage_wow.pkl')
    torch.save(train_episodes, data_path + 'train_wow.pkl')
    torch.save(dev_episodes, data_path + 'dev_wow.pkl')
    torch.save(test_seen_episodes, data_path + 'test_seen_wow.pkl')
    torch.save(test_unseen_episodes, data_path + 'test_unseen_wow.pkl')
